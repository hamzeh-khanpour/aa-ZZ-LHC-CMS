...
def build_cut_label(truncate_on: str, mzz_cut: Optional[float], shat_cut: Optional[float]) -> str:
    ...
    if truncate_on in ("mzz", "both") and mzz_cut is not None:
        pieces.append(f"m_{{ZZ}} #leq {mzz_cut:.0f} GeV")
    if truncate_on in ("shat", "both") and shat_cut is not None:
        pieces.append(f"#sqrt{{#hat{{s}}}} #leq {shat_cut:.0f} GeV")
...
def main() -> int:
    ...
    if args.truncate_on != "none":
        ...
        summary_lines.extend([
            f"Truncation mode: {args.truncate_on}",
            f"Cut used: {cut_label}",
            f"Passing events: {n_pass} / {n_all} = {frac_pass:.6f}",
            f"Estimated truncated cross section: {sigma_pass_pb:.8e} pb",
        ])
        ...
        trunc_info = info_lines + [
            f"Passing fraction = {frac_pass:.6f}",
            f"#sigma_{{trunc}} = {sigma_pass_pb:.8e} pb",
        ]
        ...
        clipped_info = [
            ...
            f"Truth/event-level clipping: {cut_label}",
            f"#sigma_{{trunc}} = {sigma_pass_pb:.8e} pb",
        ]
        draw_and_save_1d(
            h_pt_norm_cut,
    ...
    if args.truncate_on != "none":
        print("Saved truncated/clipped files include:")
        for name in [
